...
compile_command::compile_command(lyra::cli &cli)
{
    cli.add_argument(lyra::command("compile", [this](const lyra::group &) { this->run(); })
                         ...
                         .add_argument(lyra::opt(split_w_scale_).name("--split-w-scale").optional().help("balance weights value into act value or not if possible, default is " + std::to_string(split_w_scale_)))
