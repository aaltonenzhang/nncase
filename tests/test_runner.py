            ...
            for item in self.pre_process:
                ...
                if 'input_range' in item.keys() and 'input_shape' in item.keys() and 'model_shape' in item.keys():
                    ...
                    if model_shape[1] != data.shape[1] or model_shape[2] != data.shape[2]:
                        ...
                        resize_shape = data.shape[0], int(round(in_h * ratio)), int(round(in_w * ratio)), 3
                        ...
                        data = cv2.copyMakeBorder(resize_data, int(round(dh - 0.1)), int(round(model_h - resize_shape[1] - int(round(dh - 0.1)))), int(round(dw - 0.1)), int(round(
                            model_w - resize_shape[2] - int(round(dw - 0.1)))), cv2.BORDER_CONSTANT, value=(item['letterbox_value'], item['letterbox_value'], item['letterbox_value']))
